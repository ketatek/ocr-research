...
class MarkItDownOCR:
    ...
    def process_pdf(self, pdf_path: str, output_path: Optional[str] = None) -> Dict[str, any]:
        ...
        try:
            ...
            logger.success(f"Successfully processed PDF with MarkItDown")
            logger.info(f"Extracted text length: {len(text_content)} characters")

        except Exception as e:
            logger.error(f"Error processing PDF with MarkItDown: {e}")
            result['error'] = str(e)
    ...
    def process_multiple_pdfs(self, pdf_dir: str, output_dir: str) -> Dict[str, Dict]:
        ...
        for pdf_file in pdf_files:
            output_file = os.path.join(
                output_dir,
                f"{pdf_file.stem}_markitdown.txt"
            )
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage: python markitdown_ocr.py <pdf_path> [output_path]")
        sys.exit(1)
